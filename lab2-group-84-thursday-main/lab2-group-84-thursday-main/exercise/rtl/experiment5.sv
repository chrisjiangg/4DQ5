...
logic [7:0] topline [15:0];
logic [7:0] bottomline [15:0];
logic [3:0] ones;
logic [3:0] tens;

logic[3:0] firstnineposition;
logic[3:0] lastzeroposition;
logic firstnineset;
logic lastzeroset;
...
PS2_to_LCD_ROM	PS2_to_LCD_ROM_inst (
	.address ( {mode, data_reg[15]} ), //repalced with mode
	.clock ( CLOCK_50_I ),
	.q ( LCD_code )
	);
...
always_ff @ (posedge CLOCK_50_I or negedge resetn) begin
	if (resetn == 1'b0) begin
		mode <= 1'b0; //start in lowercase mode
		ones <= 7'h7f; //nine position
		tens <= 7'h7f; //zero position
		firstnineset <= 1'd0;
		lastzeroset <= 1'd0;
		firstnineposition <= 4'd0;
		lastzeroposition <= 4'd0;
		led0 <= 1'b1;
		state <= S_LCD_INIT;
		LCD_init_index <= 3'd0;
		LCD_start <= 1'b0;
		...
		S_IDLE: begin
			...
			if (PS2_code_ready && ~PS2_code_ready_buf && PS2_make_code == 1'b1) begin
				if(PS2_code == 8'h12) begin //uppercase mode
					mode <= 1'b1;
				end else if (PS2_code == 8'h59) begin //lowercase mode
					mode <= 1'b0;
				end else begin
					if(LCD_line == 1'b0) begin
						if(data_counter < 4'd15) begin
							topline[4'd15-data_counter] <= {mode,PS2_code};
							if(PS2_code == 8'h46 && firstnineset == 1'd0) begin
								firstnineset <= 1'd1;
								ones <= counter % 10;
								tens <= counter / 10;
							end
							...
							data_counter <= 4'd0;
							counter <= 4'd15;
							if(firstnineset) firstnineset <= 1'b0;
					...
					data_reg[0] <= {mode, PS2_code}; //adds the mode infront of ps2 code to reference memory correctly
				end
			end
		end
		...
		S_LCD_ISSUE_CHANGE_LINE: begin
			// Change line
			LCD_instruction <= {4'b01, ~LCD_line, 6'h00};
			LCD_line <= ~LCD_line;
			LCD_start <= 1'b1;
		...
		S_LCD_FINISH_CHANGE_LINE: begin
			if (LCD_start == 1'b1) begin
				LCD_start <= 1'b0;
				firstnineset <= 1'b0; //reset flags here
			end else begin	
				if (LCD_done == 1'b1) begin	
					if (data_counter < 4'd15) begin
						data_counter <= data_counter + 4'd1;
