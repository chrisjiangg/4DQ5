...
assign w_k = read_data_a[0];
assign x_k = read_data_a[1];

assign w_k_sign = {w_k[7], w_k};
assign x_k_sign = {x_k[7], x_k};
...
always_comb begin
	if(read_address <= 9'd255) begin
		...
		if(x_k[7] == 1'b0) begin
			y_k = w_k_sign + x_k_sign;
			z_k = w_k_sign - x_k_sign;
		end else begin
			y_k = x_k_sign - w_k_sign;
			z_k = x_k_sign + w_k_sign;
		end
	end
	
	write_data_b[0] = y_k[8:0];
	write_data_b[1] = z_k[8:0];
end
