				...
				if (pixel_Y_pos >= VIEW_AREA_TOP && pixel_Y_pos < VIEW_AREA_BOTTOM) begin
					if (pixel_X_pos == (VIEW_AREA_LEFT - 3)) begin
						if (pixel_Y_pos == VIEW_AREA_TOP) begin
							...
							// Provide address for data 1
							//SRAM_address <= 18'd0;
							SRAM_address <= RED_BASE_ADDRESS + (even_pixel_counter << 1);
//							even_pixel_counter <= 18'd0;
			...
			S_NEW_PIXEL_ROW_DELAY_3: begin		
				SRAM_address <=  (even_pixel_counter >> 1) + RED_BASE_ADDRESS +1;
			...
			S_NEW_PIXEL_ROW_DELAY_7: begin
				...
				even_pixel_counter <= even_pixel_counter + 1'd1;
            ...
            SRAM_address <= EVEN_GREEN_BASE_ADDRESS + even_pixel_counter;
            state <= S_FETCH_PIXEL_DATA_2;
        	end

        	S_FETCH_PIXEL_DATA_2: begin           	 
            SRAM_address <= EVEN_BLUE_BASE_ADDRESS + even_pixel_counter;
            VGA_red <= VGA_sram_data[0][7:0];
            VGA_green <= VGA_sram_data[4][15:8];
            VGA_blue <= VGA_sram_data[5][15:8];
            ...
            SRAM_address <= ODD_GREEN_BASE_ADDRESS + even_pixel_counter;
            VGA_sram_data[1] <= SRAM_read_data;
            VGA_red <= VGA_sram_data[3][15:8];
            ...
            SRAM_address <= ODD_BLUE_BASE_ADDRESS + even_pixel_counter;
            VGA_sram_data[2] <= SRAM_read_data;
            ...
            VGA_sram_data[3] <= SRAM_read_data;
            VGA_red <= VGA_sram_data[3][7:0];    
            VGA_green <= VGA_sram_data[4][7:0];
            VGA_blue <= VGA_sram_data[5][7:0];
            state <= S_FETCH_PIXEL_DATA_7;
			end

			S_FETCH_PIXEL_DATA_7: begin
				 // Store the last read data for the current pixel
				 VGA_sram_data[4] <= SRAM_read_data;
			    even_pixel_counter <= even_pixel_counter + 18'd1;
				 // Check if within the row boundary
				 if (pixel_X_pos < (VIEW_AREA_RIGHT - 4)) begin
					  // Increment for the next set of even pixels
					  even_pixel_counter <= even_pixel_counter + 18'd1;
...
//					  even_pixel_counter <= 1'd0;
					  // Reset VGA outputs at row end to prevent residual colors
					  VGA_red <= 8'd0;
					  VGA_green <= 8'd0;
					  VGA_blue <= 8'd0;
